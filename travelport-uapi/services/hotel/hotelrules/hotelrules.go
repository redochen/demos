...
//HotelRulesAsync 异步获取酒店规则接口
func HotelRulesAsync(param *HotelRulesParam) *HotelRulesResult {
	defer CheckPanic()
	...
	ch := make(chan *HotelRulesResult, 1)

	//异步执行
	go func(p *HotelRulesParam, c chan<- *HotelRulesResult) {
		c <- HotelRules(p)
	}(param, ch)

	var result *HotelRulesResult
	...
	case <-time.After(timeoutSeconds * time.Second):
		...
		Logger.Error(param.LogContext, " hotel rules search timed out!!!")
		break
	}
...
//HotelRules 获取酒店规则接口
func HotelRules(param *HotelRulesParam) *HotelRulesResult {
	defer CheckPanic()
	...
	//反序列化响应XML文件
	var rspEnvelope hotproxy.HotelRulesRspEnvelope
	...
	if err != nil {
		Logger.Error(param.LogContext, err.Error())
		return SetErrorCode(ErrNoDataReturned)
	}
