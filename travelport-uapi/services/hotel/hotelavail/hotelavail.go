...
func HotelAvailAsync(param *HotelAvailParam) *HotelAvailResult {
	defer CheckPanic()
	...
	ch := make(chan *HotelAvailResult, 1)

	//异步执行
	go func(p *HotelAvailParam, c chan<- *HotelAvailResult) {
		c <- HotelAvail(p)
	}(param, ch)

	var result *HotelAvailResult
...
//HotelAvail 酒店查询接口
func HotelAvail(param *HotelAvailParam) *HotelAvailResult {
	defer CheckPanic()
	...
	//反序列化响应XML文件
	var rspEnvelope hotproxy.HotelAvailRspEnvelope
